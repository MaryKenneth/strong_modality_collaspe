"""
INVERTED ASYMMETRIC FUSION (IAF) + MAKD — 4 COMBINATIONS (MultiHUSE)
==============================================================================
"""
...
OUTPUT_DIR      = 'predictions_output_inverted_asymmetric_distilled'
...
COMBINATIONS = [
    {
        ...
        'text_checkpoint':  'multihuse_distill_bert_from_e5',   # BERTStudent distilled from E5
    ...
    },
    {
        ...
        'text_checkpoint':  'text_e5_text_embeddings',           # E5 baseline (already teacher)
...
print(f"\n{'='*70}")
print(f"INVERTED ASYMMETRIC FUSION — DISTILLED MODELS (MultiHUSE)")
print(f"{'='*70}")
...
class TextModel(nn.Module):
    """E5 baseline text model (1024 → 512 → NUM_CLASSES). Used for e5 combinations."""
...
def load_text_model(combination, fold):
    """Load correct text model depending on combination type."""
    ckpt = f"{CHECKPOINTS_DIR}/{combination['text_checkpoint']}_fold_{fold}.pth"

    if combination['text_type'] == 'bert_student':
        model = BERTStudent().to(device)
    else:  # e5_baseline
        model = TextModel(combination['text_dim']).to(device)

    model.load_state_dict(torch.load(ckpt, map_location=device))
    print(f"  Text  loaded ({combination['text_type']}): {ckpt}")
    return model
...
class FusionDataset(Dataset):
    ...
    def __init__(self, t, a, v, y, fnames):
        ...
        self.v      = torch.tensor(v, dtype=torch.float32) if not isinstance(v, torch.Tensor) else v
...
all_results = []
fold_timings = []   # per-fold wall-clock timing records, across all combinations

for combination in COMBINATIONS:
    print(f"\n{'='*80}")
    print(f"COMBINATION: {combination['name'].upper()}  [DISTILLED MODELS]")
    print(f"{'='*80}")
    print(f"Text:  {combination['text_features']} — {combination['text_type']}")
    print(f"Audio: dasheng_audio_embeddings.csv — AudioStudent (distilled from E5)")
    print(f"Video: {combination['video_features']} — VideoStudent (distilled from E5)")
    print(f"{'='*80}\n")
    ...
    for fold in range(1, 6):
        ...
        # Load frozen models
        print("Loading distilled models...")
        txt_model = load_text_model(combination, fold)

        aud_model = AudioStudent().to(device)
        aud_model.load_state_dict(torch.load(
            f"{CHECKPOINTS_DIR}/{combination['audio_checkpoint']}_fold_{fold}.pth",
            map_location=device))
        print(f"  Audio loaded: {combination['audio_checkpoint']}_fold_{fold}.pth")

        vid_model = VideoStudent(combination['video_dim']).to(device)
        vid_model.load_state_dict(torch.load(
            f"{CHECKPOINTS_DIR}/{combination['video_checkpoint']}_fold_{fold}.pth",
            map_location=device))
        print(f"  Video loaded: {combination['video_checkpoint']}_fold_{fold}.pth")
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc > best_acc:
                ...
                torch.save(model.state_dict(),
                           f"{CHECKPOINTS_DIR}/inverted_asymmetric_distilled_"
            ...
            if (epoch + 1) % 20 == 0:
                ...
                print(f"  Epoch {epoch+1} [{phase}]: Train={train_acc:.2f}% "
                      ...
                      f"EpochTime={epoch_times[-1]:.3f}s")

            if trigger_times >= PATIENCE:
                print(f"  Early stopping at epoch {epoch+1}")
                break

        fold_wall_time = time.time() - fold_train_start
        mean_epoch_time = float(np.mean(epoch_times))
        n_epochs_run = len(epoch_times)

        # inference-only timing pass 
        infer_start = time.time()
        _ = evaluate(model, val_loader)
        infer_time = time.time() - infer_start
        n_val = len(val_ds)
        infer_ms_per_sample = 1000 * infer_time / n_val if n_val else float('nan')

        print(f"Fold {fold} Best Acc: {best_acc:.2f}%  "
              f"MeanEpochTime={mean_epoch_time:.3f}s ({n_epochs_run} epochs)  "
              f"Inference={infer_ms_per_sample:.3f}ms/sample")
        ...
        fold_timings.append({
            ...
            'fold_wall_time_sec': fold_wall_time,
            'inference_time_sec_full_val_set': infer_time,
    ...
    combo_timings = [t for t in fold_timings if t['combination'] == combination['name']]
    mean_epoch_time_combo = np.mean([t['mean_epoch_time_sec'] for t in combo_timings])
    mean_infer_ms_combo   = np.mean([t['inference_ms_per_sample'] for t in combo_timings])

    print(f"\n{'='*80}")
    print(f"COMBINATION {combination['name'].upper()} [DISTILLED] — FINAL RESULTS")
    print(f"{'='*80}")
    print(f"Mean Accuracy: {mean_acc:.2f}% (±{std_acc:.2f}%)")
    print(f"Mean epoch time across folds: {mean_epoch_time_combo:.3f}s")
    print(f"Mean inference time across folds: {mean_infer_ms_combo:.3f}ms/sample")

    combo_df = pd.DataFrame(all_fold_preds)
    combo_df.to_csv(
        f"{OUTPUT_DIR}/inverted_asymmetric_distilled_{combination['name']}_predictions.csv",
        index=False)
    ...
    all_results.append({
        ...
        'mean_epoch_time_sec': mean_epoch_time_combo,
        'mean_inference_ms_per_sample': mean_infer_ms_combo,
    })

# ── Final summary ──────────────────────────────────────────────────────────
print(f"\n{'='*80}")
print("INVERTED ASYMMETRIC FUSION (DISTILLED MODELS) — ALL COMBINATIONS SUMMARY")
print(f"{'='*80}")
results_df = pd.DataFrame(all_results)
print(results_df.to_string(index=False))
results_df.to_csv(f"{OUTPUT_DIR}/inverted_asymmetric_distilled_summary.csv", index=False)
pd.DataFrame(fold_timings).to_csv(f"{OUTPUT_DIR}/inverted_asymmetric_distilled_timing_per_fold.csv", index=False)
print(f"\nSummary saved to {OUTPUT_DIR}/inverted_asymmetric_distilled_summary.csv")
print(f"Per-fold timing saved to {OUTPUT_DIR}/inverted_asymmetric_distilled_timing_per_fold.csv")
print(f"\n{'─'*50}")
print("Compare against baseline script to measure distillation gain:")
print("  Baseline fusion  → fusion_inverted_asymmetric_baseline_models.py")
print("  Distilled fusion → this script")
print(f"{'='*80}")
