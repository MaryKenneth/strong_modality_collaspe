"""
SYMMETRIC FUSION + MAKD - 4 COMBINATIONS (MultiHUSE)
====================================================================
"""
...
OUTPUT_DIR      = 'predictions_output_symmetric_distilled'
...
print(f"\n{'='*70}")
print(f"SYMMETRIC FUSION — DISTILLED MODELS (MultiHUSE)")
print(f"{'='*70}")


# ==========================================================================
# DISTILLED STUDENT MODEL DEFINITIONS 

class TextModel(nn.Module):
    """E5 baseline text model (1024 → 512 → NUM_CLASSES). Used for E5 combos."""
...
class BERTStudent(nn.Module):
    """BERT student distilled from E5 (768 → 512 → 512 encoder). Used for BERT combos."""
    def __init__(self):
        super().__init__()
...
def load_text_model(combo, fold):
    """Load the correct text model depending on text_type."""
    if combo['text_type'] == 'bert_student':
...
for combo in COMBINATIONS:
    ...
    print(f"{'='*70}")
    print(f"Text:  {combo['text_features']} [{combo['text_type']}] (dim: {combo['text_dim']})")
    print(f"Audio: dasheng_audio_embeddings.csv [distilled] (dim: {AUDIO_DIM})")
    print(f"Video: {combo['video_features']} [distilled] (dim: {combo['video_dim']})")
    print(f"{'='*70}\n")
    ...
    for fold in range(1, 6):
        ...
        # Load frozen distilled backbones
        print("Loading distilled backbone models...")
        txt_model = load_text_model(combo, fold)
        print(f"  Text  loaded: {combo['text_checkpoint']}_fold_{fold}.pth [{combo['text_type']}]")
        ...
        aud_model.load_state_dict(torch.load(
            ...
            map_location=device))
        print(f"  Audio loaded: {combo['audio_checkpoint']}_fold_{fold}.pth")

        vid_model = VideoStudent(combo['video_dim']).to(device)
        vid_model.load_state_dict(torch.load(
            f"{CHECKPOINTS_DIR}/{combo['video_checkpoint']}_fold_{fold}.pth",
            map_location=device))
        print(f"  Video loaded: {combo['video_checkpoint']}_fold_{fold}.pth")
        ...
        ckpt_path = f"{CHECKPOINTS_DIR}/symmetric_distilled_{combo['name']}_fold_{fold}.pth"

        epoch_times = []       # wall-clock seconds for train_one_epoch, per epoch
        ...
        n_test = len(trues)
        infer_ms_per_sample = 1000 * infer_time / n_test if n_test else float('nan')
        ...
        fold_timings.append({
            ...
            'inference_time_sec_full_test_set': infer_time,
    ...
    combo_timings = [t for t in fold_timings if t['combination'] == combo['name']]
    ...
    print(f"\nMean epoch time across folds: {mean_epoch_time_combo:.3f}s")
...
print(f"\n{'='*70}")
print(f"SYMMETRIC DISTILLED — ALL COMBINATIONS SUMMARY")
print(f"{'='*70}")
for r in all_results:
    print(f"  {r['combination']:30s}  {r['mean_accuracy']:.2f}% (±{r['std_accuracy']:.2f}%)  "
          f"EpochTime={r['mean_epoch_time_sec']:.3f}s  "
          f"Inference={r['mean_inference_ms_per_sample']:.3f}ms/sample")
print(f"{'='*70}")

pd.DataFrame(all_results).to_csv(f"{OUTPUT_DIR}/symmetric_distilled_timing_summary.csv", index=False)
pd.DataFrame(fold_timings).to_csv(f"{OUTPUT_DIR}/symmetric_distilled_timing_per_fold.csv", index=False)
print(f"Timing summary saved to {OUTPUT_DIR}/symmetric_distilled_timing_summary.csv")
