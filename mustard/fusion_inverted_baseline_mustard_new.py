"""
INVERTED ASYMMETRIC FUSION – BASELINE MODELS (MUStARD)
======================================================
Uses baseline (non-distilled) text models with inverted asymmetric fusion.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
BASELINE_CONFIG = 'e5_pe_core'   # ← CHANGE: 'bert_videomae', 'bert_pe_core',
                                    #           'e5_videomae', 'e5_pe_core'

CONFIGS = {
    'bert_videomae': {
        'name': 'BERT (Baseline) + Audio + VideoMAE',
    ...
    'bert_pe_core': {
        'name': 'BERT (Baseline) + Audio + Video_PE',
        'text_type': 'bert_context_punchline',
        'text_checkpoint': 'mustard_text_bert_context_punchline',
        'text_dim': 1536,
        'video_type': 'pe_core',
    ...
    'e5_videomae': {
        'name': 'E5 (Baseline) + Audio + VideoMAE',
    ...
    'e5_pe_core': {
        'name': 'E5 (Baseline) + Audio + Video_PE',
        'text_type': 'e5_context_punchline',
        'text_checkpoint': 'mustard_text_e5_context_punchline',
        'text_dim': 2048,
        'video_type': 'pe_core',
...
if BASELINE_CONFIG not in CONFIGS:
    raise ValueError(f"Invalid BASELINE_CONFIG. Must be one of {list(CONFIGS.keys())}")

CONFIG = CONFIGS[BASELINE_CONFIG]
print(f"\n{'='*70}")
print(f"BASELINE INVERTED FUSION: {CONFIG['name']}")
print(f"{'='*70}")
print(f"Text: {CONFIG['text_type']} (baseline, context+punchline)")
...
OUTPUT_DIR = f'predictions_output_fusion_inverted_baseline_{BASELINE_CONFIG}'
...
for fold in range(5):
    ...
    text_baseline = TextBaseline(CONFIG['text_dim']).to(device)
    text_baseline.load_state_dict(
    ...
    text_train_feats,  audio_train_feats,  video_train_feats  = extract_backbone_features(
        text_baseline, audio_baseline, video_baseline,
    ...
    # Build fusion model
    model = InvertedAsymmetricFusion(text_baseline, audio_baseline, video_baseline).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(),
                       f"{CHECKPOINTS_DIR}/fusion_inverted_baseline_{BASELINE_CONFIG}_fold_{fold}.pth")
        else:
            trigger_times += 1
    ...
    model.load_state_dict(
        torch.load(f"{CHECKPOINTS_DIR}/fusion_inverted_baseline_{BASELINE_CONFIG}_fold_{fold}.pth",
                   map_location=device))
    ...
    fold_weights.append(weights)   # collect per fold
    ...
    print(f"Fold {fold} Final: {acc:.2f}%  W[T={weights[0]:.3f} A={weights[1]:.3f} V={weights[2]:.3f}]")
...
pd.DataFrame(all_pred_rows).to_csv(
    f"{OUTPUT_DIR}/predictions_{BASELINE_CONFIG}.csv", index=False)

pd.DataFrame(fold_summary_rows).to_csv(
    f"{OUTPUT_DIR}/fold_summary_{BASELINE_CONFIG}.csv", index=False)

report_dict = classification_report(
    all_trues, all_preds, target_names=['not_sarcasm', 'sarcasm'],
    output_dict=True, zero_division=0)
pd.DataFrame(report_dict).transpose().to_csv(
    f"{OUTPUT_DIR}/classification_report_{BASELINE_CONFIG}.csv")
...
with open(f"{OUTPUT_DIR}/summary_{BASELINE_CONFIG}.json", 'w') as f:
